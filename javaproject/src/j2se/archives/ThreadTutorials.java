...
/*
 * Class: ThreadTutorials Created on Apr 6, 2013 (c) Copyright Chetan Subhash
 * Patil., unpublished work, All use, disclosure, and/or reproduction of this
 * material is prohibited unless authorized in writing. All Rights Reserved.
...
public class ThreadTutorials
{
   ...
   * thread that will sleep, not the t thread. t.suspend() is deprecated. Using
   * it is possible to halt a thread other than the current thread. A suspended
   * thread keeps all its monitors and since this state is not interruptable it
   * is deadlock prone. object.wait() sends the current thread into the
   * "Not Runnable" state, like sleep(), but with a twist. Wait is called on a
   * object, not a thread; we call this object the "lock object." Before
   * lock.wait() is called, the current thread must synchronize on the lock
   * object; wait() then releases this lock, and adds the thread to the
   * "wait list" associated with the lock. Later, another thread can synchronize
   * on the same lock object and call lock.notify(). This wakes up the original,
   * waiting thread. Basically, wait()/notify() is like sleep()/interrupt(),
   * only the active thread does not need a direct pointer to the sleeping
   ...
   * particular action is in this state. TIMED_WAITING A thread that is waiting
   * for another thread to perform an action for up to a specified waiting time
   * is in this state. TERMINATED A thread that has exited is in this state. A
   * thread can be in only one state at a given point in time. These states are
  ...
  {
    ThreadTutorials main = new ThreadTutorials();
    main.createProcess();
    main.createThread();
  }
